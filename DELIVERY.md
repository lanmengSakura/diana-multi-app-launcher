...
本次公开测试包为 `0.1.0-beta.4-rc.4`，修复状态查询阻塞并补齐交付流程，保留 rc.3 本机应用关联和此前完整运行组件。请下载本次 Release 的新 EXE 并核对 SHA-256；旧版不会自动更新。仍为预发布版本，不作全场景或跨版本兼容承诺。详见 [Issue #1 回执](docs/ISSUE_1_RECEIPT.md)。
