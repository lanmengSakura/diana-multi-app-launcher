...
for (const [from, to] of [["index.html", "_diana-shell.page"], ["launcher.html", "_diana-launcher.page"], ["themes.html", "_diana-themes.page"], ["theme-app.html", "_diana-theme-app.page"]]) {
...
writeFileSync(new URL("dist/server/wrangler.json", root), JSON.stringify({
  ...
  assets: { directory: "../client", binding: "ASSETS", html_handling: "auto-trailing-slash", not_found_handling: "404-page", run_worker_first: ["/", "/index.html", "/launcher", "/launcher.html", "/launcher/", "/themes", "/themes.html", "/themes/", "/theme-app", "/theme-app.html", "/theme-app/"] }
}, null, 2));
for (const file of ["dist/client/_diana-shell.page", "dist/client/_diana-launcher.page", "dist/client/_diana-themes.page", "dist/client/_diana-theme-app.page", "dist/server/index.js", ".openai/hosting.json"]) {
